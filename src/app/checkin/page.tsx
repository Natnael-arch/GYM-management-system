  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col p-8">
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative">
        ...
        {!result ? (
          <div className="text-center text-gray-400 mt-16">
            ...
            <h1 className="text-4xl font-bold text-gray-500 tracking-widest uppercase">Ready to Scan</h1>
          </div>
        ) : (
            ...
            {result.allowed && result.member ? (
              <div className="flex items-center justify-center space-x-12 relative">
                {result.isOfflineQueued && (
                  <div className="absolute -top-8 bg-yellow-400 text-yellow-900 font-bold px-4 py-1 rounded-full uppercase text-xs tracking-wider shadow">
                    Queued Offline
                  </div>
                )}
                {result.member.photoUrl ? (
                  <img src={result.member.photoUrl} alt="Photo" className="w-64 h-64 rounded-full object-cover shadow-xl border-8 border-green-500" />
                ) : (
                  <div className="w-64 h-64 rounded-full bg-green-700 flex items-center justify-center text-green-300 text-3xl font-bold border-8 border-green-500">
                    No Photo
                  </div>
                )}
                <div className="text-left">
                  <h1 className="text-7xl font-bold mb-4 drop-shadow-md">{result.member.name}</h1>
                  <h2 className="text-4xl font-semibold text-green-200">Access Granted</h2>
                  <p className="text-xl mt-4 text-green-100">Membership active until {result.member.membershipEndsAt?.includes('Valid') ? result.member.membershipEndsAt : format(new Date(result.member.membershipEndsAt!), 'MMM d, yyyy')}</p>
                </div>
              </div>
            ) : (
              <div className="py-16 relative">
                {result.reason === 'GYM_CLOSED' && (
                  <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-yellow-400 text-yellow-900 font-bold px-4 py-1 rounded-full uppercase tracking-wider shadow">
                    LOCKDOWN ACTIVE
                  </div>
                )}
                <svg className="w-32 h-32 mx-auto mb-6 text-red-300 drop-shadow-md" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <h1 className="text-6xl font-bold mb-6 drop-shadow-md">ACCESS DENIED</h1>
                <h2 className="text-4xl font-bold text-red-200 uppercase px-8">{getReasonText(result.reason)}</h2>
              </div>
            )}
          </div>
        )}
      ...
      <div className="mt-8 border-t border-gray-800 pt-8">
        <h3 className="text-gray-500 uppercase tracking-wider text-sm font-bold mb-4">Recent Scans</h3>
        <div className="flex gap-4 overflow-x-auto pb-4">
          {history.length === 0 && <span className="text-gray-700">No recent scans.</span>}
          {history.map((h, i) => (
            <div key={i} className={`flex-shrink-0 w-64 p-4 rounded-xl border ${h.allowed ? 'border-green-800 bg-green-900/30' : 'border-red-800 bg-red-900/30'}`}>
              <div className="flex justify-between items-start mb-2">
                <span className={`text-xs font-bold px-2 py-1 rounded ${h.allowed ? 'bg-green-800 text-green-100' : 'bg-red-800 text-red-100'}`}>
                  {h.allowed ? (h.isOfflineQueued ? 'QUEUED' : 'ALLOW') : 'DENY'}
                </span>
                <span className="text-xs text-gray-500">{format(h.scannedAt, 'HH:mm:ss')}</span>
              </div>
