...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Find if they currently have an active membership to determine start date (renew vs issue)
    const activeMembership = await prisma.membership.findFirst({
      where: {
        memberId,
        ...getActiveMembershipQuery(gymCurrentTime)
      },
      orderBy: { endsAt: 'desc' }
    });
    ...
    // Create the membership (and optionally the payment) inside a transaction
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (paymentAmountCents !== undefined && paymentMethod) {
